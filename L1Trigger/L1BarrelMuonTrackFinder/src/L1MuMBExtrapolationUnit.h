...
//
//--------------------------------------------------
#ifndef L1MUMB_EXTRAPOLATION_UNIT_H
#define L1MUMB_EXTRAPOLATION_UNIT_H
...
class L1MuMBExtrapolationUnit : public L1AbstractProcessor {

  public:

    typedef std::pair<Extrapolation, unsigned int> SEUId;
    typedef std::map<SEUId, L1MuMBSEU*, std::less<SEUId> > SEUmap;

    /// constructor
    L1MuMBExtrapolationUnit(const L1MuMBSectorProcessor& );

    /// destructor
    virtual ~L1MuMBExtrapolationUnit();
  ...
  private:
    ...
    const L1MuMBSectorProcessor& m_sp;   // reference to Sector Processor
