...
L1MuMBMuonSorter::L1MuMBMuonSorter(const L1MuMBTrackFinder& tf) :
...
L1MuMBMuonSorter::~L1MuMBMuonSorter() {
...
// run MB Muon Sorter
//
void L1MuMBMuonSorter::run() {

  // get track candidates from Wedge Sorters
  vector<L1MuMBTrack*> mycands;
  mycands.reserve(24);

  for ( int wedge = 0; wedge < 12; wedge++ ) {
    vector<const L1MuMBTrack*> wscand = m_tf.ws(wedge)->tracks();
    vector<const L1MuMBTrack*>::iterator iter = wscand.begin();
    while ( iter != wscand.end() ) {
      if ( *iter && !(*iter)->empty() )
        mycands.push_back(const_cast<L1MuMBTrack*>(*iter) );
      iter++;
    }
  }
  ...
  if ( L1MuMBTFConfig::Debug(4) ) {
    cout << "MB Muon Sorter input: "
         << mycands.size() << endl;
    vector<L1MuMBTrack*>::const_iterator iter;
  ...
  vector<L1MuMBTrack*>::iterator it = mycands.begin();
  ...
  // sort pt and quality
  stable_sort( mycands.begin(), mycands.end(), L1MuMBTrack::Rank() );

  // copy the best 4 candidates
  int number_of_tracks = 0;
  vector<L1MuMBTrack*>::const_iterator iter1 = mycands.begin();
...
//
// reset MB Muon Sorter
//
void L1MuMBMuonSorter::reset() {

  m_TrackCands.clear();
  vector<const L1MuMBTrack*>::iterator iter;
...
//
// print candidates found in the MB Muon Sorter
//
void L1MuMBMuonSorter::print() const {

  cout << endl;
  cout << "Muon candidates found by the barrel MTTF : "
       << numberOfTracks() << endl;
  vector<const L1MuMBTrack*>::const_iterator iter = m_TrackCands.begin();
...
//
// Cancel Out Logic for MB Muon Sorter
//
void L1MuMBMuonSorter::runCOL(vector<L1MuMBTrack*>& cands) const {
...
//
int L1MuMBMuonSorter::neighbour(const L1MuMBSecProcId& spid1,
                                const L1MuMBSecProcId& spid2) {
