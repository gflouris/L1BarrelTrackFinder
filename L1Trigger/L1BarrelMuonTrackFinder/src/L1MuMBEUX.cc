...
L1MuMBEUX::L1MuMBEUX(const L1MuMBSectorProcessor& sp, const L1MuMBSEU& seu, int id) :
    m_sp(sp), m_seu(seu), m_id(id),
    m_result(false), m_quality(0), m_address(15),
    m_start(0), m_target(0),
    theExtFilter(L1MuMBTFConfig::getExtTSFilter()),
    nbit_phi(L1MuMBTFConfig::getNbitsExtPhi()),
    nbit_phib(L1MuMBTFConfig::getNbitsExtPhib())
{
}
...
// Equal operator
//
bool L1MuMBEUX::operator==(const L1MuMBEUX& eux) const {
...
// run EUX
//
void L1MuMBEUX::run(const edm::EventSetup& c) {

  c.get< L1MuDTExtLutRcd >().get( theExtLUTs );
  c.get< L1MuDTTFParametersRcd >().get( pars );

  if ( L1MuMBTFConfig::Debug(4) ) cout << "Run EUX "  << m_id << endl;
  if ( L1MuMBTFConfig::Debug(4) ) cout << "start :  " << *m_start  << endl;
  if ( L1MuMBTFConfig::Debug(4) ) cout << "target : " << *m_target << endl;

  if ( m_start == 0 || m_target == 0 ) {
    if ( L1MuMBTFConfig::Debug(4) ) cout << "Error: EUX has no data loaded" << endl;
    return;
  }
  ...
  if ( L1MuMBTFConfig::Debug(5) ) cout << "EUX : using look-up table : "
  ...
  if (( diff >= low && diff <= high ) || L1MuMBTFConfig::getopenLUTs() || openlut ) {
    m_result = true;
  ...
  if ( L1MuMBTFConfig::Debug(5) ) cout << "diff : "   << low  << " "
                                       << diff << " " << high << " : "
                                       << m_result << " " << endl;
...
// load data into EUX
//
void L1MuMBEUX::load(const L1MuMBTrackSegPhi* start_ts,
                     const L1MuMBTrackSegPhi* target_ts) {
...
//
pair<const L1MuMBTrackSegPhi* ,const L1MuMBTrackSegPhi*> L1MuMBEUX::ts() const {

  return pair<const L1MuMBTrackSegPhi*, const L1MuMBTrackSegPhi*>(m_start,m_target);
...
//
int L1MuMBEUX::sec_mod(int sector) const {
