...
// Constructors --
//----------------
L1MuMBSectorReceiver::L1MuMBSectorReceiver(L1MuMBSectorProcessor& sp) :
        m_sp(sp) {
...
// Destructor --
//--------------
L1MuMBSectorReceiver::~L1MuMBSectorReceiver() {
...
//
// receive track segment data from the MBBX and CSC chamber triggers
//
void L1MuMBSectorReceiver::run(int bx, const edm::Event& e, const edm::EventSetup& c) {

  c.get< L1MuDTTFParametersRcd >().get( pars );
  c.get< L1MuDTTFMasksRcd >().get( msks );

  // get track segments from MBBX chamber trigger
  receiveMBBXData(bx, e, c);

  // get track segments from CSC chamber trigger
  if ( L1MuMBTFConfig::overlap() && m_sp.ovl() ) {
...
// clear
//
void L1MuMBSectorReceiver::reset() {

}


//
// receive track segment data from the MBBX chamber trigger
//
void L1MuMBSectorReceiver::receiveMBBXData(int bx, const edm::Event& e, const edm::EventSetup& c) {
  ...
  for ( int station = 1; station <= 4; station++ ) {
    ...
    for (int reladr =0; reladr < max_address; reladr++) {
      ...
      if ( ts ) {
        ...
        if ( L1MuMBTFConfig::getTSOutOfTimeFilter() || nbx_del ) {

          int sh_phi = 12 - L1MuMBTFConfig::getNbitsExtPhi();
          int tolerance = L1MuMBTFConfig::getTSOutOfTimeWindow();
        ...
        if ( !skipTS ) {
          L1MuMBTrackSegPhi tmpts(wheel,sector,station,phi,phib,
                                  static_cast<L1MuMBTrackSegPhi::TSQuality>(qual),
                                  tag,bx-bx_offset);
...
//
void L1MuMBSectorReceiver::receiveCSCData(int bx, const edm::Event& e, const edm::EventSetup& c) {

  if ( (L1MuMBTFConfig::getCSCTrSInputTag()).label() == "none" ) return;

  if ( bx < -6 || bx > 6 ) return;

  edm::Handle<CSCTriggerContainer<csctf::TrackStub> > csctrig;
  e.getByLabel(L1MuMBTFConfig::getCSCTrSInputTag(),csctrig);
  ...
  for ( csc_iter = csc_list.begin(); csc_iter != csc_list.end(); csc_iter++ ) {
    ...
    // convert CSC quality code to MBBX quality code
    unsigned int qual = 7;
    if ( qualCSC ==  2 ) qual = 0;
    if ( qualCSC ==  6 ) qual = 1;
    if ( qualCSC ==  7 ) qual = 2;
    ...
    if ( L1MuMBTFConfig::getEtaCanc() && etaFlag ) continue;

    if ( ncsc < 2 ) {
      int address = 16 + ncsc;
      bool tag = (ncsc == 1 ) ? true : false;
      L1MuMBTrackSegPhi tmpts(wheel,sector,station+2,phi,0,
                              static_cast<L1MuMBTrackSegPhi::TSQuality>(qual),
                              tag,bx,etaFlag);
...
//
int L1MuMBSectorReceiver::address2sector(int adr) const {
...
//
int L1MuMBSectorReceiver::address2wheel(int adr) const {
