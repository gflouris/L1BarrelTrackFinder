...
L1MuMBWedgeSorter::L1MuMBWedgeSorter(const L1MuMBTrackFinder& tf, int id) :
...
L1MuMBWedgeSorter::~L1MuMBWedgeSorter() {
...
// run Wedge Sorter
//
void L1MuMBWedgeSorter::run() {

  // get track candidates from Sector Processors
  vector<L1MuMBTrack*> wedgecands;
  ...
  for ( int wheel = -3; wheel <= 3; wheel++ ) {
    ...
    for ( int number = 0; number < 2; number++ ) {
      ...
      if ( cand && !cand->empty() ) {
        ...
        if ( !reject ) wedgecands.push_back(const_cast<L1MuMBTrack*>(cand));
      }
    }
  }
  ...
  vector<L1MuMBTrack*>::iterator it = wedgecands.begin();
...
//
void L1MuMBWedgeSorter::print() const {
...
//
bool L1MuMBWedgeSorter::anyTrack() const {
...
//
void L1MuMBWedgeSorter::runCOL(vector<L1MuMBTrack*>& cands) const {
...
//
bool L1MuMBWedgeSorter::neighbour(const L1MuMBSecProcId& spid1,
