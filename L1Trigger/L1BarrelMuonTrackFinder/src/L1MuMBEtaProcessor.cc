...
L1MuMBEtaProcessor::L1MuMBEtaProcessor(const L1MuMBTrackFinder& tf, int id) :
...
L1MuMBEtaProcessor::~L1MuMBEtaProcessor() {}
...
// run Eta Processor
//
void L1MuMBEtaProcessor::run(int bx, const edm::Event& e, const edm::EventSetup& c) {

  if ( L1MuMBTFConfig::getEtaTF() ) {
    receiveData(bx,e,c);
    runEtaTrackFinder(c);
  }
...
// reset Eta Processor
//
void L1MuMBEtaProcessor::reset() {

  vector<const L1MuMBTrackSegEta*>::iterator iter = m_tseta.begin();
...
//
void L1MuMBEtaProcessor::print() const {
...
//
// receive data ( 15*3 MBBX eta trigger primitives )
//
void L1MuMBEtaProcessor::receiveData(int bx, const edm::Event& e, const edm::EventSetup& c) {
  ...
  for ( int stat = 1; stat <= 3; stat++ ) {
    for ( int wheel = -2; wheel <= 2; wheel++ ) {
      ...
      const L1MuMBTrackSegEta* tmpts = new L1MuMBTrackSegEta(wheel,sector,stat,pos.to_ulong(),qual.to_ulong(),bx-bx_offset);
...
//
void L1MuMBEtaProcessor::receiveAddresses() {
  ...
  for ( int wheel = -3; wheel <= 3; wheel++ ) {
    if ( wheel == 0 ) continue;
    L1MuMBSecProcId tmpspid(wheel,sector);
    for ( int number = 0; number < 2; number++ ) {
      const L1MuMBTrack* cand = m_tf.sp(tmpspid)->track(number);
      const L1MuMBTrack* canD = m_tf.sp(tmpspid)->tracK(number);
      if ( cand ) {
        m_address[i] = cand->address().trackAddressCode();
        if ( !cand->empty() ) {
          m_TrackCand[i] = const_cast<L1MuMBTrack*>(cand);
          m_TracKCand[i] = const_cast<L1MuMBTrack*>(canD);
        }
        i++;
      }
...
//
void L1MuMBEtaProcessor::runEtaTrackFinder(const edm::EventSetup& c) {
...
//
void L1MuMBEtaProcessor::runEtaMatchingUnit(const edm::EventSetup& c) {
...
//
void L1MuMBEtaProcessor::assign() {
  ...
  for ( int i = 0; i < 12; i++ ) {
	...
	//        if ( i/2 != 2 ) cerr << "L1MuMBEtaProcessor: assign invalid eta" << " " << m_address[i] << endl;
      }
      if ( m_fine[i] ) {
        ...
        const L1MuDTEtaPattern p = theEtaPatternLUT->getPattern(m_pattern[i]);
        vector<const L1MuMBTrackSegEta*> TSeta;
        const L1MuMBTrackSegEta* ts = 0;
...
//
int L1MuMBEtaProcessor::quality(int id, int stat) {
