...
L1MuMBExtrapolationUnit::L1MuMBExtrapolationUnit(const L1MuMBSectorProcessor& sp) :
                    m_sp(sp), m_SEUs() {
  ...
  for ( int ext_idx = 0; ext_idx < MAX_EXT; ext_idx++ ) {
    ...
    if ( ext == EX12 || ext == EX13 || ext == EX14 ||
      ...
      for ( unsigned int startAddress = 0; startAddress < maxaddr; startAddress++ ) {
        ...
        m_SEUs[id] = new L1MuMBSEU(sp,ext,startAddress);
        if ( L1MuMBTFConfig::Debug(6) ) cout << "  creating SEU " << ext << " " << startAddress << endl;
      }
    }
  }
...
L1MuMBExtrapolationUnit::~L1MuMBExtrapolationUnit() {
...
//
void L1MuMBExtrapolationUnit::run(const edm::EventSetup& c) {
  ...
  if ( L1MuMBTFConfig::getUseEX21() || run_21 ) {
    ...
    for ( unsigned int startAdr = 0; startAdr < 2; startAdr++ ) {
      ...
      for ( int eux = 0; eux < 12; eux++ ) {
        if ( extab12.test(eux) && !extab21.test(eux) ) {
          ...
          if ( L1MuMBTFConfig::Debug(6) ) {
            SEUId seuid = make_pair(EX12, startAdr);
            L1MuMBSEU* SEU12 = m_SEUs[seuid];
...
//
void L1MuMBExtrapolationUnit::reset() {
...
//
void L1MuMBExtrapolationUnit::reset(Extrapolation ext, unsigned int startAdr, unsigned int relAdr) {
...
//
unsigned short int L1MuMBExtrapolationUnit::getAddress(Extrapolation ext, unsigned int startAdr, int id) const {
...
//
unsigned short int L1MuMBExtrapolationUnit::getQuality(Extrapolation ext, unsigned int startAdr, int id) const {
...
//
const bitset<12>& L1MuMBExtrapolationUnit::getEXTable(Extrapolation ext, unsigned int startAdr) const {
...
//
const bitset<12>& L1MuMBExtrapolationUnit::getQSTable(Extrapolation ext, unsigned int startAdr) const {
...
//
int L1MuMBExtrapolationUnit::numberOfExt() const {
...
//
void L1MuMBExtrapolationUnit::print(int level) const {

  SEUmap::const_iterator iter_seu;

  if ( level == 0 ) {
    for ( iter_seu = m_SEUs.begin(); iter_seu != m_SEUs.end(); iter_seu++ ) {
      vector<L1MuMBEUX*> vec_eux = ((*iter_seu).second)->eux();
...
//
pair<int,int> L1MuMBExtrapolationUnit::which_ext(Extrapolation ext) {
