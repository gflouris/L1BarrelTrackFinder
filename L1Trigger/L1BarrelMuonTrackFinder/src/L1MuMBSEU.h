...
class L1MuMBSEU : public L1AbstractProcessor {

  public:

    /// constructor
    L1MuMBSEU(const L1MuMBSectorProcessor& sp, Extrapolation ext, unsigned int tsId );
    ...
    inline void load(const L1MuMBTrackSegPhi* startTS) { m_startTS = startTS; }
    ...
    inline const std::vector<L1MuMBEUX*>& eux() const { return m_EUXs; }

    /// return pointer to Extrapolation Result Selector
    inline const L1MuMBERS* ers() const { return m_ERS; }
  ...
  private:
    ...
    const L1MuMBTrackSegPhi*     m_startTS;     // start track segment
    std::vector<L1MuMBEUX*>      m_EUXs;        // vector of Extrapolators
    L1MuMBERS*                   m_ERS;         // Extrapolation Result Selector
