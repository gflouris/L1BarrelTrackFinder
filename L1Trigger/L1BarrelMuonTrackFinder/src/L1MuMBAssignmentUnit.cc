...
#include "L1MuMBAssignmentUnit.h"
...
#include "L1MuMBTFConfig.h"
#include "L1MuMBSectorProcessor.h"
#include "L1MuMBDataBuffer.h"
#include "L1MuMBTrackSegPhi.h"
#include "L1MuMBTrackSegLoc.h"
#include "L1MuMBTrackAssembler.h"
#include "L1MuMBTrackAssParam.h"
...
L1MuMBAssignmentUnit::L1MuMBAssignmentUnit(L1MuMBSectorProcessor& sp, int id) :
                m_sp(sp), m_id(id),
...
L1MuMBAssignmentUnit::~L1MuMBAssignmentUnit() {}
...
// run Assignment Unit
//
void L1MuMBAssignmentUnit::run(const edm::EventSetup& c) {
  ...
  vector<const L1MuMBTrackSegPhi*>::const_iterator iter = m_TSphi.begin();
  int bx = (*iter)->bx();
...
//
void L1MuMBAssignmentUnit::reset() {
...
//
void L1MuMBAssignmentUnit::PhiAU(const edm::EventSetup& c) {

  // calculate phi at station 2 using 8 bits (precision = 2.5 degrees)

  //c.get< L1MuMBPhiLutRcd >().get( thePhiLUTs );
  int phi_load = thePhiLUTs->load();
    if ( phi_load != 0 ) {
      cout << "Can not open files to load pt-assignment look-up tables for MBTrackFinder!" << endl;
    }

  int sh_phi  = 12 - L1MuMBTFConfig::getNbitsPhiPhi();
  int sh_phib = 10 - L1MuMBTFConfig::getNbitsPhiPhib();

  const L1MuMBTrackSegPhi* second = getTSphi(2);  // track segment at station 2
  const L1MuMBTrackSegPhi* first  = getTSphi(1);  // track segment at station 1
  const L1MuMBTrackSegPhi* forth  = getTSphi(4);  // track segment at station 4
...
//
void L1MuMBAssignmentUnit::PtAU(const edm::EventSetup& c) {

//  c.get< L1MuMBPtaLutRcd >().get( thePtaLUTs );
...
//
void L1MuMBAssignmentUnit::QuaAU() {
...
//
void L1MuMBAssignmentUnit::TSR() {

  // get the track segments from the data buffer
  const L1MuMBTrackSegPhi* ts = 0;
...
//
const L1MuMBTrackSegPhi* L1MuMBAssignmentUnit::getTSphi(int station) const {
...
//
int L1MuMBAssignmentUnit::convertSector(int sector) {
...
// determine charge
//
int L1MuMBAssignmentUnit::getCharge(PtAssMethod method) {
...
//
PtAssMethod L1MuMBAssignmentUnit::getPtMethod() const {
  ...
  switch ( method ) {
    ...
    default : ;
      //cout << "L1MuMBAssignmentUnit : Error in PT ass method evaluation" << endl;
  }
...
//
int L1MuMBAssignmentUnit::getPtAddress(PtAssMethod method, int bendcharge) const {
...
//
int L1MuMBAssignmentUnit::phiDiff(int stat1, int stat2) const {
...
//
void L1MuMBAssignmentUnit::setPrecision() {

  nbit_phi  = L1MuMBTFConfig::getNbitsPtaPhi();
  nbit_phib = L1MuMBTFConfig::getNbitsPtaPhib();
...
unsigned short int L1MuMBAssignmentUnit::nbit_phi  = 12;
unsigned short int L1MuMBAssignmentUnit::nbit_phib = 10;
